...
export class GasPriceService {
  ...
  async getGasPrice() {
    try {
      ...
      const response = await this.httpService.axiosRef.get<GasPriceResponse>(url, requestConfig);
      await this.cacheManager.set('gasPrice', BigInt(parseInt(response.data.result, 16)));
