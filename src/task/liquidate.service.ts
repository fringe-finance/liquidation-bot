...
import { LiquidateParams, TokenType } from 'src/contract/dto/liquidate-params.dto';
...
export class LiquidateService {
  ...
  constructor(
    ...
    if (enableCheckProfit) {
      this.enableCheckProfit = enableCheckProfit.trim() === 'true' ? true : false;
    } else {
  ...
  private async liquidate(chainNetworkId: number, liquidatablePosition: LiquidatablePosition) {
    try {
      const liquidationContractAddress = this.configService.get(`LIQUIDATION_ADDRESS`);
      this.logService.log('----------------------------------------------');
      this.logService.log('Liquidate for position: ', liquidatablePosition);
      let totalDebt = BigInt(liquidatablePosition.lendingTokenOutstandingCount);
      ...
      const updatePriceData = await this.priceAggregatorService.getUpdatePriceData(
        ...
        [
          ...liquidatablePosition.collateralToken.underlyingTokens,
          ...liquidatablePosition.lendingToken.underlyingTokens,
        ],
      );

      const exchangeType = await this.liquidationContractService.getExchangeAggregatorType(
        chainNetworkId,
      ...
      if (
        ((this.enableCheckProfit && isHaveProfit) || !this.enableCheckProfit) &&
  ...
  private async estimateTransactionFee(chainId: number, contractAddress: string, params: LiquidateParams) {
    const gasPrice = await this.gasPriceService.getGasPrice();
    const gasLimit = await this.liquidationContractService.estimateGas(chainId, contractAddress, params);
  ...
  private async isHaveProfit(
    ...
    const estimatedCollateralReward = await this.liquidationContractService.getEstimatedCollateralReward(
      chainNetworkId,
      liquidationContractAddress,
    ...
    const profitInUSD = uint256ToNumber(BigInt(estimatedCollateralReward), this.usdDecimals);

    const nativePrice = await this.tokenPriceService.getNativeCoinPrice(chainNetworkId);
