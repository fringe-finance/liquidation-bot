...
import { PlpService } from 'src/contract/plp.service';
...
export class LiquidateService {
    ...
    constructor(
        ...
        private readonly plpService: PlpService,
    ...
    async handleCron() {
        ...
        if (
            ...
            for (let j = 0; j < liquidatablePositions.length; j++) {
                ...
                if (
                    liquidatablePosition.liquidatorRewardFactor - 1 <
                    this.configService.get('MAX_DISCREPANCY')
                ) {
                    this.logService.log('Liquidator reward factor is too low');
                    continue;
                }
    ...
    private async liquidate(
        ...
        try {
            const liquidationContractAddress = this.configService.get(
                `PLP_LIQUIDATION_CONTRACT_ADDRESS`,
            );
            this.logService.log(
                '----------------------------------------------',
            );
            ...
            const tokensUpdateFinalPrice =
                await this.plpService.getTokensUpdateFinalPrices(
                    chainNetworkId,
                    this.configService.get('PLP_CONTRACT_ADDRESS'),
                    liquidatablePosition.collateralToken.address,
                    liquidatablePosition.lendingToken.address,
                );

            let tokens = [
                ...tokensUpdateFinalPrice,
                ...liquidatablePosition.collateralToken.underlyingAssets,
                ...liquidatablePosition.lendingToken.underlyingAssets,
            ...
            const totalDebt = BigInt(
                liquidatablePosition.totalOutstandingAmount,
            );
            ...
            const { bLendingToken } = await this.plpService.getbLendingToken(
                chainNetworkId,
                this.configService.get('PLP_CONTRACT_ADDRESS'),
                liquidatablePosition.lendingToken.address,
            );
            ...
            const liquidatorAddress =
                await this.liquidationContractService.getLiquidatorAddress(
                    chainNetworkId,
                );

            const allowanceAmount = await this.erc20Service.allowance(
                chainNetworkId,
                liquidatablePosition.lendingToken.address,
                liquidatorAddress,
                bLendingToken,
            ...
            const { minLA, maxLA } =
                await this.liquidationContractService.getLiquidationAmount(
                    chainNetworkId,
                    liquidationContractAddress,
                    liquidatablePosition.borrowerAddress,
                    liquidatablePosition.collateralToken.address,
                    liquidatablePosition.lendingToken.address,
                    updatePriceData.priceIds,
                    updatePriceData.updateData,
                    BigInt(updatePriceData.updateFee),
                );
            ...
            const targetAllowance =
                (lendingAmount * BigInt(100 + 5)) / BigInt(100);
            const targetValue = BigInt(
                targetAllowance.toString().split('.')[0],
            );
            if (allowanceAmount < targetValue) {
                await this.erc20Service.approve(
                    chainNetworkId,
                    liquidatablePosition.lendingToken.address,
                    bLendingToken,
                    targetValue,
        ...
        const liquidateParam: LiquidateParams = {
            ...
            collateralInfo: {
                ...
                tokenType:
                    TokenType[
                        liquidatablePosition.collateralToken
                            .tokenType as keyof typeof TokenType
                    ],
            },
            lendingInfo: {
