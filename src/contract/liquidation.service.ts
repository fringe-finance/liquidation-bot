...
export class LiquidationContractService extends BaseContractService {
  ...
  async estimateGas(chainId: number, contractAddress: string, params: LiquidateParams) {
    try {
      const liquidationContract = await this.getContractWrite(chainId, contractAddress);
      ...
      this.logService.error('Error when estimate liquidate position: ', e);
    }
  }
  ...
  async getEstimatedCollateralReward(
    ...
    try {
      const liquidationContract = await this.getContractRead(chainId, liquidationAddress);
      if (!liquidationContract) {
        this.logService.log("Can't get contract instance");
        return;
      }
      ...
      const { assets, assetAmounts } = await liquidationContract.liquidate.staticCall(tuple, {
        value: params.updateFee,
      });
      ...
      if (
        ...
        const { collateralEvaluation } = await this.plpService.getTokenEvaluation(
          chainId,
          plpAddress,
          assets[0],
          assetAmounts[0],
      ...
      this.logService.error('Error when get amount collateral required: ', e);
    }
  }

  async getExchangeAggregatorType(chainId: number, contractAddress: string): Promise<ExchangeType> {
    try {
      const liquidationContract = await this.getContractRead(chainId, contractAddress);
      if (!liquidationContract) {
        this.logService.log("Can't get contract instance");
        return;
      }

      const registryAggregator = await liquidationContract.registryAggregator();
