...
export class PriceAggregatorService extends BaseContractService {
  ...
  async getUpdatePriceData(
    ...
    for (const token of listTokens) {
      const isUsingPythOracle = await this.isUsingPythOracle(networkId, contractAddress, token);
    ...
    const connection = new EvmPriceServiceConnection(this.config.get('PYTHNET_PRICE_FEED_ENDPOINT'));
