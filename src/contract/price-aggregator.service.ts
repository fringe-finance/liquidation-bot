...
import * as PRICE_AGGREGATOR_ABI from './abi/PriceAggregator.json';
...
export class PriceAggregatorService extends BaseContractService {
    ...
    async isUsingPythOracle(
        ...
        try {
            ...
            const priceProvider = await contract.tokenPriceProvider(
                tokenAddress,
            );
            return (
                this.config.get('PYTH_PRICE_PROVIDER_CONTRACT_ADDRESS') ===
                priceProvider
            );
        } catch (error) {
            if (
                error.message ===
                'Returned error: You reached the free tier limits, please upgrade your tier to paid one'
            ) {
            } else {
    ...
    async getExpiredPriceFeeds(
        networkId: number,
        contractAddress: string,
        listToken: string[],
    ): Promise<ExpiredPriceFeed> {
        try {
            const contract = await this.getContractRead(
                networkId,
                contractAddress,
            );
            return await contract.getExpiredPriceFeeds(
                listToken,
                this.config.get('TIME_BEFORE_EXPIRATION'),
            );
        } catch (error) {
    ...
    async getUpdatePriceData(
        networkId: number,
        contractAddress: string,
        listTokens: string[],
    ): Promise<UpdatePriceData> {
        ...
        try {
            ...
            throw new Error(
                `Error when get price feeds update data from pythnet service`,
            );
        }
