...
export class ExchangeAggregatorService {
  ...
  async buyOnDex(
    ...
    const tokenSrcInfo = await this.erc20Service.tokenInfo(chainId, tokenSrc);
    const tokenDestInfo = await this.erc20Service.tokenInfo(chainId, tokenDest);
  ...
  async estimate(
    ...
    const tokenTypeIn = AssetType[tokenIn.tokenType as keyof typeof AssetType];
    const tokenTypeOut = AssetType[tokenOut.tokenType as keyof typeof AssetType];
    if (tokenTypeIn === AssetType.ERC20 && tokenTypeOut === AssetType.ERC20) {
    ...
    } else if (tokenTypeIn === AssetType.ERC20 && tokenTypeOut === AssetType.ERC4626) {
    ...
    } else if (tokenTypeIn === AssetType.ERC4626 && tokenTypeOut === AssetType.ERC20) {
    ...
    } else if (tokenTypeIn === AssetType.ERC20 && tokenTypeOut === AssetType.LP) {
    ...
    } else if (tokenTypeIn === AssetType.LP && tokenTypeOut === AssetType.ERC20) {
    ...
    } else if (tokenTypeIn === AssetType.ERC4626 && tokenTypeOut === AssetType.ERC4626) {
    ...
    } else if (tokenTypeIn === AssetType.LP && tokenTypeOut === AssetType.ERC4626) {
    ...
    } else if (tokenTypeIn === AssetType.ERC4626 && tokenTypeOut === AssetType.LP) {
  ...
  async estimateBuyLPFromERC20(
    ...
    const lpAcceptableAmount = getMaxDiscrepancyAmount(BigInt(expectedAmountOut), maxDiscrepancy);
    const lpToken0DesiredAmount =
      (BigInt(lpAcceptableAmount) * BigInt(lpToken0Reserve)) / BigInt(lpTotalSupply);
    const lpToken1DesiredAmount =
      (BigInt(lpAcceptableAmount) * BigInt(lpToken1Reserve)) / BigInt(lpTotalSupply);

    const { buyData0: lpToken0BuyData, buyData1: lpToken1BuyData } = await this.estimateBuyOnDexLP(
      tokenIn.address,
      lpToken0Address,
    ...
    const erc20AmountEstimated = BigInt(lpToken0BuyData.amountIn) + BigInt(lpToken1BuyData.amountIn);
  ...
  async estimateBuyERC20FromLP(
    ...
    const { sellData0: lpToken0SellData, sellData1: lpToken1SellData } = await this.estimateSellOnDexLP(
      tokenOut.address,
      lpToken0Address,
      erc20EstimatedAmountForToken0,
      lpToken1Address,
    ...
    const lpEstimatedAmountWithToken0 =
      (BigInt(lpToken0EstimatedAmount) * BigInt(lpTotalSupply)) / BigInt(lpToken0Reserve);
    const lpEstimatedAmountWithToken1 =
      (BigInt(lpToken1EstimatedAmount) * BigInt(lpTotalSupply)) / BigInt(lpToken1Reserve);
    const lpEstimatedAmount =
      BigInt(lpEstimatedAmountWithToken0) > BigInt(lpEstimatedAmountWithToken1)
    ...
    return {
      ...
      expectedAmountOut: expectedAmountOut,
      buyCallData: [lpToken0SellData?.buyCallData, lpToken1SellData?.buyCallData].filter((data) => !!data),
    };
  }
