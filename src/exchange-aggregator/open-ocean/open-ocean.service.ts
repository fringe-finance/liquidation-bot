...
export class OpenOceanService {
  ...
  async createCallDataOpenOcean(
    ...
    const { data: priceData } = await this.httpService.axiosRef.get(`${this.baseUrl}/${chainId}/swap_quote`, {
      params: {
        chain: chainId,
        inTokenAddress: srcToken,
        outTokenAddress: destToken,
        amount: uint256ToNumber(BigInt(srcAmount), Number(srcDecimals)),
