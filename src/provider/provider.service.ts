...
export class ProvidersService {
  ...
  protected chainIdToProvider: Map<number, ethers.JsonRpcProvider> = new Map();
  ...
  constructor(private readonly configService: ConfigService, private readonly logService: LogService) {
    const networkIds = stringToArray(this.configService.get('NETWORK_ID'), ',');
    networkIds.forEach((chainId: string, index: number) => {
      const rpcList = stringToArray(this.configService.get(`NETWORK_RPC`), ',');
      this.chainIdToRpcUrl.set(Number(chainId), rpcList);
    });
    this.chainNetworkIds = networkIds.map((id) => Number(id));
    this.walletPrivateKey = this.configService.get('LIQUIDATOR_PRIVATE_KEY');
  }
